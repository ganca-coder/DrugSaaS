...
- **Koupleless 热更新**：基于 SOFA Koupleless（`koupleless-runtime` 2.1.11），服务可作为「基座 + 业务模块（biz）」方式部署，业务模块独立打包、独立发布、**热更新无需重启基座进程**，显著降低发布停机时间与回归范围。

### 部署方式总览

| 编号 | 部署方式 | 适用场景 | 部署单元 | 说明 |
|---|---|---|---|---|
| 1 | 单点快速部署 | 开发调试 / 演示 / 单店试点 | 单个 JAR（全量合并） | 全部服务合并为一个进程 |
| 2 | 简单多服务部署 | 小连锁 / 资源受限 | 2~3 个进程 | 网关 + 应用层 + 服务层合并 |
| 3 | 简单层级部署 | 中型连锁 | 按层 + 大类合并 | 网关 / 应用层 / 服务层（按大类） |
| 4 | 细分微服务部署 | 大型连锁 / 多租户高并发 | 每个领域服务独立进程 | 全部服务独立部署 |
| 5 | 部分服务合并部署 | 按实际负载裁剪 | 混合 | 低频服务合并、高频服务独立 |
