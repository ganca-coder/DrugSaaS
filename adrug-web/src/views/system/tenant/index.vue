<template>
  <div class="page">
    <el-card shadow="never">
      <el-form :inline="true" :model="query" class="search-form" @submit.prevent>
        <el-form-item label="关键字">
          <el-input v-model="query.keyword" placeholder="租户名称/编码" clearable style="width: 200px" @keyup.enter="handleSearch" />
        </el-form-item>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit ? '编辑租户' : '新增租户'" width="560px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        <el-row :gutter="12">
          <el-col :span="12"><el-form-item label="租户编码" prop="tenantCode"><el-input v-model="form.tenantCode" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="租户名称" prop="tenantName"><el-input v-model="form.tenantName" /></el-form-item></el-col>
          <el-col :span="12">
            <el-form-item label="状态">
...
import { pageTenant, createTenant, updateTenant, deleteTenant } from '@/api/tenant'

defineOptions({ name: 'system-tenant' })
...
const formRef = ref()
const createEmptyForm = () => ({ id: null, tenantCode: '', tenantName: '', status: 1, contactName: '', contactPhone: '', remark: '' })
...
async function load() {
  loading.value = true
  try {
    const data = await pageTenant({ ...query })
...
function openEdit(row) {
  dialog.isEdit = true
  form.value = { id: row.id, tenantCode: row.tenantCode, tenantName: row.tenantName, status: row.status, contactName: row.contactName, contactPhone: row.contactPhone, remark: row.remark }
  dialog.visible = true
}
